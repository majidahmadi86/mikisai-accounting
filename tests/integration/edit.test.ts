...
suite("editing a row through the app's update path", () => {
  ...
  beforeAll(async () => {
    ...
    const tf = await admin.from("internal_transfers").insert({ business_id: SEED_BUSINESS_ID, date: "2026-09-16", from_person: "sai", to_person: "mike", amount: 50, kind: "settlement", reason: "profit_share", note: "edit-probe", created_by: mikeId, created_at: stamp }).select("id").single();
  ...
  it("changes a transfer's date the same way", async () => {
    const outcome = await applyTransferUpdate(mike, SEED_BUSINESS_ID, ids.transfer, { date: "2026-09-15", from_person: "sai", to_person: "mike", amount: 50, kind: "settlement", reason: "profit_share", note: "edit-probe" });
    expect(outcome).toEqual({ ok: true });
