...
export default async function EditTransactionPage({ params, searchParams }: PageProps<"/transactions/[id]/edit">) {
  ...
  const [{ data }, { data: auditRows }, { data: profiles }] = await Promise.all([
    supabase.from("transactions").select("*, settlements(status)").eq("id", id).maybeSingle(),
    supabase.from("audit_log").select("*").eq("entity_type", "transaction").eq("entity_id", id).order("created_at", { ascending: false }).limit(20),
      ...
      <Card className="mt-4">
        ...
        <div className="px-5 pb-5 sm:px-6 sm:pb-6">
          <p className="text-sm text-plum-soft">
            {lastEdit
              ? tr("transactions.editedBy", { name: nameOf.get(lastEdit.actor_user_id) ?? "?", when: formatDateTime(lastEdit.created_at, locale) })
              : created
                ? tr("transactions.addedBy", { name: nameOf.get(created.actor_user_id) ?? "?", when: formatDateTime(created.created_at, locale) })
              ...
              {history.map((h) => (
                <li key={h.id} className="py-2.5 text-sm">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-plum">{nameOf.get(h.actor_user_id) ?? "?"}</span>
